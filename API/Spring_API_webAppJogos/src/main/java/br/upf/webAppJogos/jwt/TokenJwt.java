package br.upf.webAppJogos.jwt;
...
public class TokenJwt {
	...
	public String gerarToken(String nomeUsuario, Date dataExpiracao) {
		jwt = Jwts.builder().setHeaderParam("typ", "JWT") // definindo cabeçalho
				.setSubject(nomeUsuario) // assunto do token
				.setIssuer("upf") // quem é o emissor do token
				.setIssuedAt(new Date()) // data de criação
				.claim("password", "sdlkjsdoijonpvf65v4e6fv5e6ver").setExpiration(dataExpiracao) // data de expiração do
																									// token
